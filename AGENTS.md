...
- Project command runner: `./pmake` (lists targets); use it for build, run, stop, test, lint, and format.
- Frontend dependencies: `cd frontend && npm install`
- Validate Compose example wiring: `docker compose --env-file .env.example config`
- Enable version-controlled Git hooks: `./scripts/install-git-hooks.sh`
...
- Use `./pmake` for backend build work.
...
- During early development, when migrations are squashed or deleted, run `./pmake test-backend-integration` so stale copied migrations are removed from `target/classes` and integration tests see the current schema.
