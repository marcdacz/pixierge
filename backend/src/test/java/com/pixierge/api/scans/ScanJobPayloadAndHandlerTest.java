...
class ScanJobPayloadAndHandlerTest {
    ...
    void identityPayloadBuildsSingletonItemForLegacyPayloads() {
        ...
        assertThat(items).containsExactly(new ScanIdentityJobPayload.ScanIdentityJobItem(
                rootId,
                assetFileId,
                "/photos/one.jpg",
                "/normalized/photos/one.jpg",
    ...
    void identityPayloadUsesBatchItemsForCompletionRootWhenRootIsMissing() {
        ...
        UUID libraryId = UUID.randomUUID();
        UUID rootId = UUID.randomUUID();
        List<ScanIdentityJobPayload.ScanIdentityJobItem> items = List.of(
                item(rootId, "one.jpg"),
                item(rootId, "two.jpg")
        );
        ScanIdentityJobPayload payload = new ScanIdentityJobPayload(
    ...
    void identityPayloadReturnsNoCompletionRootForIncompleteOrMixedItems() {
        UUID scanRunId = UUID.randomUUID();
        UUID libraryId = UUID.randomUUID();
        ScanIdentityJobPayload incomplete = new ScanIdentityJobPayload(
                scanRunId,
                libraryId,
                UUID.randomUUID(),
                null,
        ...
        ScanIdentityJobPayload mixed = new ScanIdentityJobPayload(
                scanRunId,
                libraryId,
                null,
    ...
    void subtreeHandlerDelegatesHandleAndCompletionToScanService() throws Exception {
        ...
        ScanCatalogSubtreeJobHandler handler = new ScanCatalogSubtreeJobHandler(scanService, objectMapper);
        ScanCatalogJobPayload payload = new ScanCatalogJobPayload(
                UUID.randomUUID(),
                UUID.randomUUID(),
                UUID.randomUUID(),
                "/normalized/photos"
