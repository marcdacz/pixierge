...
class BackgroundJobServiceTest {
  ...
  private final BackgroundJobService service =
      new BackgroundJobService(repository, new ImmediateTransactionTemplate(), CLOCK);
  ...
  void workerCompletesJobWithRegisteredHandler() {
    ...
    assertThat(repository.find(jobId).orElseThrow().status())
        .isEqualTo(BackgroundJobRepository.STATUS_SUCCEEDED);
  }
  ...
  void claimSkipsSecondJobWithSameConcurrencyKey() {
    ...
    assertThat(repository.find(firstId).orElseThrow().status())
        .isEqualTo(BackgroundJobRepository.STATUS_RUNNING);
    assertThat(repository.find(secondId).orElseThrow().status())
        .isEqualTo(BackgroundJobRepository.STATUS_PENDING);
  }
  ...
  private static class FakeBackgroundJobRepository extends BackgroundJobRepository {
    ...
    public List<BackgroundJobRecord> claimReadyJobs(
      ...
      return ready.stream().map(job -> find(job.id()).orElseThrow()).toList();
    }
    ...
    public List<BackgroundJobStatusSummary> summarizeByTypeAndStatus() {
      return jobs.stream()
          .collect(
              java.util.stream.Collectors.groupingBy(job -> job.jobType() + "\n" + job.status()))
          .entrySet()
          .stream()
          .map(
              entry -> {
    ...
    public List<BackgroundJobProblemSummary> latestProblemJobs(int limit) {
      return jobs.stream()
          .filter(
              job ->
                  STATUS_FAILED.equals(job.status())
