...
class BackgroundFileActivityPagerTest {
  ...
  void pageZeroPrependsActiveRowsAndCountsThemTowardPageSize() {
    List<BackgroundFileActivitySummary> active =
        List.of(active("a.jpg", "processing"), active("b.jpg", "pending"));
    ...
    BackgroundFileActivityPage page =
        BackgroundFileActivityPager.page(
            active,
            0,
            3,
            (offset, limit) -> {
              ...
              return new BackgroundActivityRepository.PersistedFileActivityPage(
                  List.of(row("/photos/c.jpg", "added", NOW.minusMinutes(1))), 5);
            });
    ...
    assertThat(page.items())
        .extracting(BackgroundFileActivitySummary::fileName)
  ...
  void matchesFiltersActiveRowsByQueryStatusAndDate() {
    ...
    assertThat(BackgroundFileActivityPager.matches(item, "lake", List.of("processing"), null, null))
        .isTrue();
    assertThat(BackgroundFileActivityPager.matches(item, "mountain", null, null, null)).isFalse();
    assertThat(BackgroundFileActivityPager.matches(item, null, List.of("failed"), null, null))
        .isFalse();
    assertThat(BackgroundFileActivityPager.matches(item, null, null, NOW.plusMinutes(1), null))
        .isFalse();
    assertThat(
  ...
  private static BackgroundFileActivityRow row(
      String path, String result, OffsetDateTime observedAt) {
