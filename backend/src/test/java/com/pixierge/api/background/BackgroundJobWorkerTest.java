...
class BackgroundJobWorkerTest {
  ...
  void pollBatchFailsTerminalWhenNoHandlerIsRegistered() {
    ...
    assertThat(jobService.terminalFailures)
        .containsExactly("handler_missing:No handler registered for missing");
  ...
  void pollBatchMarksJobFailedWhenExecutorRejectsIt() {
    ...
    assertThat(jobService.failures)
        .containsExactly("executor_rejected:Background worker rejected the job");
  }
