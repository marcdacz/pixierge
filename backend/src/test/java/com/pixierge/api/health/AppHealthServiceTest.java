...
class AppHealthServiceTest {
    ...
    void currentHealthIsReadyWhenSchemaMarkerExists() {
        AppHealthService service = new AppHealthService(new StubMetadataRepository(Optional.of("v1"), false));
    ...
    void currentHealthIsUnavailableWhenSchemaMarkerIsMissing() {
        AppHealthService service = new AppHealthService(new StubMetadataRepository(Optional.empty(), false));
    ...
    void currentHealthIsUnavailableWhenRepositoryFails() {
        AppHealthService service = new AppHealthService(new StubMetadataRepository(Optional.empty(), true));
