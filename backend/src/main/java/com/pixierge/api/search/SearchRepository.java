...
class SearchRepository {
  ...
  List<SearchSuggestionResponse> suggest(
    ...
    return switch (field) {
      ...
      case TAG ->
          commaAwareNamedSuggestions(
              namedSuggestionValues(
                  TAGS.name, TAGS.ownerUserId.eq(userId), lastCommaPartial(normalized), limit),
              rawPartial);
      ...
      case EXTENSION -> {
        ...
        String lastPartial =
            lastComma >= 0 ? normalized.substring(lastComma + 1).trim() : normalized;
        String extensionPartial =
            lastPartial.startsWith(".") ? lastPartial.substring(1) : lastPartial;
  ...
  private List<String> namedSuggestionValues(
      StringPath name, Predicate owner, String partial, int limit) {
  ...
  private List<SearchSuggestionResponse> commaAwareNamedSuggestions(
      List<String> matches, String rawPartial) {
        ...
        .map(
            value -> {
              ...
              String suggestionValue =
                  completedPrefix.isEmpty() ? quoted : completedPrefix + quoted;
  ...
  private List<SearchSuggestionResponse> folderSuggestions(
      String partial, int limit, UUID userId, boolean admin) {
    List<String> paths =
        queryFactory
            .select(ASSET_FILES.normalizedPath)
            .from(ASSET_FILES)
            .join(LIBRARIES)
            ...
            .orderBy(ASSET_FILES.normalizedPath.asc())
            .limit((long) limit * 4)
            .fetch();
    ...
    return folders.stream()
        .map(value -> new SearchSuggestionResponse(quoteIfNeeded(value), value))
        .toList();
  }

  private List<SearchSuggestionResponse> distinctSuggestions(
      StringPath path, String partial, int limit) {
    ...
    return queryFactory
        ...
        .where(where)
        .groupBy(path)
        .orderBy(path.lower().asc())
        .limit(limit)
        .fetch()
        .stream()
        .map(value -> new SearchSuggestionResponse(value, value))
        .toList();
  }

  private List<SearchSuggestionResponse> cameraSuggestions(
      String partial, int limit, UUID userId, boolean admin) {
    List<String> values =
        queryFactory
            .select(ASSET_METADATA.cameraMake, ASSET_METADATA.cameraModel)
            .from(ASSET_METADATA)
            .join(ASSET_FILES)
            .on(ASSET_FILES.assetId.eq(ASSET_METADATA.assetId))
            .join(LIBRARIES)
            .on(LIBRARIES.id.eq(ASSET_FILES.libraryId))
            .leftJoin(LIBRARY_MEMBERS)
            .on(LIBRARY_MEMBERS.libraryId.eq(ASSET_FILES.libraryId))
            .where(
                ASSET_METADATA
                    .cameraMake
                    .isNotNull()
                    .or(ASSET_METADATA.cameraModel.isNotNull())
            ...
            .map(
                row ->
                    String.join(
                            " ",
                            List.of(
                                row.get(ASSET_METADATA.cameraMake) == null
                                    ? ""
                                    : row.get(ASSET_METADATA.cameraMake),
                                row.get(ASSET_METADATA.cameraModel) == null
                                    ? ""
  ...
  private List<SearchSuggestionResponse> enumSuggestions(
      List<String> values, String partial, int limit) {
    return values.stream()
  ...
  private BooleanBuilder readableAssetSuggestionWhere(UUID userId, boolean admin) {
    BooleanBuilder where =
        new BooleanBuilder(LIBRARIES.status.eq(STATUS_ACTIVE))
