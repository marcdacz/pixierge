...
public class SearchService {
  ...
  public List<SearchSuggestionResponse> suggest(
      AuthenticatedUser user, String fieldSyntax, String partial, Integer requestedLimit) {
    SearchField field =
        SearchField.fromSyntax(fieldSyntax == null ? "" : fieldSyntax.toLowerCase(Locale.ROOT))
            .orElseThrow(
                () -> new ResponseStatusException(HttpStatus.BAD_REQUEST, "Unknown search field"));
    int limit =
        Math.min(
            MAX_SUGGESTION_LIMIT,
            Math.max(1, requestedLimit == null ? DEFAULT_SUGGESTION_LIMIT : requestedLimit));
    return repository.suggest(
        field, partial, limit, user.id(), user.permissions().contains(PERMISSION_LIBRARY_ADMIN));
  }
}
