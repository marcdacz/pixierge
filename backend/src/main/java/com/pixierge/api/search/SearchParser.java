...
public class SearchParser {
  ...
  private static final Set<SearchField> DATE_FIELDS =
      EnumSet.of(SearchField.AFTER, SearchField.BEFORE, SearchField.ON);
  private static final Set<SearchField> COMMA_VALUE_FIELDS =
      EnumSet.of(SearchField.LIBRARY, SearchField.ALBUM, SearchField.TAG, SearchField.EXTENSION);

  public SearchParseResponse parseResponse(String source) {
    ParseResult result = parseInternal(source == null ? "" : source);
    List<SearchParseResponse.Clause> clauses =
        result.clauses().stream()
            .map(
                clause ->
                    new SearchParseResponse.Clause(
                        clause.field().syntax(),
                        clause.value(),
                        clause.negated(),
                        clause.start(),
    ...
    return new SearchParseResponse(
        result.source(), result.freeText(), clauses, result.errors(), result.errors().isEmpty());
  }
  ...
  private ParseResult parseInternal(String source) {
    ...
    while (cursor < source.length()) {
      ...
      if (token.error() != null) {
        errors.add(
            new SearchParseResponse.Error(
                "MALFORMED_TOKEN", token.error(), token.start(), token.end()));
        continue;
      }
      ...
      if (field == null) {
        String message =
            switch (fieldSyntax.toLowerCase(Locale.ROOT)) {
              case "person", "object" ->
                  "Search field '" + fieldSyntax + "' is unavailable until a plugin provides it";
              default -> "Unknown search field '" + fieldSyntax + "'";
            };
        errors.add(
            new SearchParseResponse.Error("UNKNOWN_FIELD", message, token.start(), token.end()));
        continue;
      }
      if (value.isBlank() || value.startsWith(":")) {
        errors.add(
            new SearchParseResponse.Error(
                "MISSING_VALUE",
                "Search field '" + field.syntax() + "' requires a value",
                token.start(),
                token.end()));
        continue;
      }
      if (!field.repeatable() && !seenSingleFields.add(field)) {
        errors.add(
            new SearchParseResponse.Error(
                "REPEATED_FIELD",
                "Search field '" + field.syntax() + "' can only be used once",
                token.start(),
                token.end()));
        continue;
      }
      ...
      for (String part : values) {
        if (DATE_FIELDS.contains(field)) {
          try {
            ...
            errors.add(
                new SearchParseResponse.Error(
                    "INVALID_DATE",
                    "Use an ISO date such as 2026-07-11",
                    token.start(),
                    token.end()));
            invalid = true;
            break;
        ...
        if (field == SearchField.IS && !IS_VALUES.contains(part.toLowerCase(Locale.ROOT))) {
          errors.add(
              new SearchParseResponse.Error(
                  "INVALID_VALUE",
                  "Supported is: values are available, duplicate, and starred",
                  token.start(),
                  token.end()));
          invalid = true;
          break;
