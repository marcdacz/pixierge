...
public class TagRepository {
  ...
  public List<TagRecord> list(UUID ownerUserId) {
    return queryFactory
        .select(
            TAGS.id, TAGS.name, TAGS.createdAt, TAGS.updatedAt, ASSET_TAGS.assetId.countDistinct())
        .from(TAGS)
        .leftJoin(ASSET_TAGS)
        .on(ASSET_TAGS.tagId.eq(TAGS.id))
        ...
        .map(
            row ->
                new TagRecord(
                    row.get(TAGS.id),
                    row.get(TAGS.name),
                    row.get(TAGS.createdAt),
                    row.get(TAGS.updatedAt),
                    Math.toIntExact(row.get(ASSET_TAGS.assetId.countDistinct()))))
        .toList();
  }
  ...
  public boolean ownsAll(UUID ownerUserId, List<UUID> tagIds) {
    Long count =
        queryFactory
            .select(TAGS.id.count())
            .from(TAGS)
            .where(TAGS.ownerUserId.eq(ownerUserId).and(TAGS.id.in(tagIds)))
            .fetchOne();
  ...
  public UUID create(UUID ownerUserId, String name, String normalizedName) {
    ...
    queryFactory
        ...
        .set(TAGS.name, name)
        .set(TAGS.normalizedName, normalizedName)
  ...
  public boolean rename(UUID id, UUID ownerUserId, String name, String normalizedName) {
    return queryFactory
            ...
            .where(TAGS.id.eq(id).and(TAGS.ownerUserId.eq(ownerUserId)))
            .execute()
        > 0;
  }
  ...
  public void add(UUID tagId, UUID assetId, UUID libraryId, UUID userId) {
    queryFactory
        .insert(ASSET_TAGS)
        .set(ASSET_TAGS.tagId, tagId)
        .set(ASSET_TAGS.assetId, assetId)
        .set(ASSET_TAGS.sourceLibraryId, libraryId)
        .set(ASSET_TAGS.addedBy, userId)
    ...
    queryFactory
        ...
        .where(
            ASSET_TAGS
                .tagId
                .in(tagIds)
                .and(ASSET_TAGS.assetId.in(assetIds))
                .and(
                    ASSET_TAGS.tagId.in(
                        queryFactory
                            .select(TAGS.id)
  ...
  public List<AssetDetailResponse.Tag> listAssetTags(UUID assetId, UUID ownerUserId) {
    return queryFactory
        ...
        .from(ASSET_TAGS)
        .join(TAGS)
        .on(TAGS.id.eq(ASSET_TAGS.tagId))
        .where(ASSET_TAGS.assetId.eq(assetId).and(TAGS.ownerUserId.eq(ownerUserId)))
        .orderBy(TAGS.name.lower().asc())
        .fetch()
        .stream()
        .map(row -> new AssetDetailResponse.Tag(row.get(TAGS.id), row.get(TAGS.name)))
        .toList();
  }

  public record TagRecord(
      UUID id, String name, OffsetDateTime createdAt, OffsetDateTime updatedAt, int assetCount) {}
}
