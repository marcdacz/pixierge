...
public class AdminIdentityController {
    ...
    UserSummaryResponse updateUser(@PathVariable UUID userId, @RequestBody UpdateUserRequest request) {
        return adminUserService.updateStatus(userId, request.active());
    }

    @DeleteMapping("/users/{userId}")
    void deleteUser(@PathVariable UUID userId, @RequestBody DeleteUserRequest request, @AuthenticationPrincipal AuthenticatedUser actor) {
