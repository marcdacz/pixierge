...
class AdminUserService {
    ...
    AdminUserService(UserRepository userRepository, SessionRepository sessionRepository, PasswordEncoder passwordEncoder) {
    ...
    UserSummaryResponse updateStatus(UUID userId, boolean active) {
        ...
        if (!active && IdentityConstants.USER_STATUS_ACTIVE.equals(user.status()) && userRepository.isLastActiveAdmin(userId)) {
            throw new ResponseStatusException(HttpStatus.CONFLICT, "At least one active administrator is required");
        }
        userRepository.updateStatus(userId, active ? IdentityConstants.USER_STATUS_ACTIVE : IdentityConstants.USER_STATUS_DISABLED);
        if (!active) {
            sessionRepository.revokeAllForUser(userId);
        }
        return userRepository.requireUserSummary(userId);
    ...
    void delete(UUID actorId, UUID userId, UUID replacementUserId) {
        ...
        if (user.roles().contains(IdentityConstants.ROLE_ADMIN) && userRepository.isLastActiveAdmin(userId)
                && !replacement.roles().contains(IdentityConstants.ROLE_ADMIN)) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "Replacement user must be an administrator");
        }
