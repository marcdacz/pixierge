...
class AlbumRepository {
  ...
  List<AlbumRecord> list(UUID userId) {
    return queryFactory
        .select(
            ...
            ALBUMS.createdAt,
            ALBUMS.updatedAt,
            ITEMS.assetId.countDistinct(),
            ITEMS.sourceLibraryId.countDistinct(),
            FILES.fileName.min())
        .from(ALBUMS)
        .leftJoin(ITEMS)
        .on(ITEMS.albumId.eq(ALBUMS.id))
        .leftJoin(FILES)
        .on(FILES.assetId.eq(ALBUMS.coverAssetId))
        ...
        .groupBy(
            ...
            ALBUMS.createdAt,
            ALBUMS.updatedAt)
        .orderBy(ALBUMS.updatedAt.desc(), ALBUMS.name.lower().asc())
        .fetch()
        .stream()
        .map(this::record)
        .toList();
  ...
  List<AlbumRecord> listShared(UUID userId) {
    return queryFactory
        ...
        .from(ALBUMS)
        .join(MEMBERS)
        .on(MEMBERS.albumId.eq(ALBUMS.id).and(MEMBERS.userId.eq(userId)))
        .leftJoin(ITEMS)
        .on(ITEMS.albumId.eq(ALBUMS.id))
        .leftJoin(FILES)
        .on(FILES.assetId.eq(ALBUMS.coverAssetId))
        .where(ALBUMS.kind.eq(AlbumKind.USER))
        .groupBy(
            ALBUMS.id,
            ALBUMS.name,
            ALBUMS.coverAssetId,
    ...
    return queryFactory
            ...
            .where(
                ALBUMS
                    .id
                    .eq(albumId)
                    .and(ALBUMS.ownerUserId.eq(userId).or(MEMBERS.userId.eq(userId))))
            .fetchFirst()
        != null;
  }
    ...
    return queryFactory
            ...
            .where(
                ALBUMS
                    .id
                    .eq(albumId)
                    .and(ALBUMS.ownerUserId.eq(userId).or(MEMBERS.memberRole.eq("editor"))))
            .fetchFirst()
        != null;
  }

  boolean owns(UUID albumId, UUID userId) {
    return queryFactory
            .selectOne()
            .from(ALBUMS)
  ...
  List<AlbumMemberRecord> members(UUID albumId) {
    return queryFactory
        .select(MEMBERS.userId, USERS.username, MEMBERS.memberRole, MEMBERS.createdAt)
        .from(MEMBERS)
        .join(USERS)
        .on(USERS.id.eq(MEMBERS.userId))
        .where(MEMBERS.albumId.eq(albumId))
        .orderBy(USERS.username.asc())
        .fetch()
        .stream()
        .map(
            row ->
                new AlbumMemberRecord(
                    row.get(MEMBERS.userId),
                    row.get(USERS.username),
                    row.get(MEMBERS.memberRole),
                    row.get(MEMBERS.createdAt)))
        .toList();
  }
  ...
  boolean activeUser(UUID userId) {
    return queryFactory
            .selectOne()
            .from(USERS)
            .where(USERS.id.eq(userId).and(USERS.status.eq("active")))
            .fetchFirst()
        != null;
  }
  ...
  List<AlbumMemberRecord> activeUsersExcept(UUID userId) {
    return queryFactory
        ...
        .from(USERS)
        .where(USERS.status.eq("active").and(USERS.id.ne(userId)))
        .orderBy(USERS.username.asc())
        .fetch()
        .stream()
        .map(row -> new AlbumMemberRecord(row.get(USERS.id), row.get(USERS.username), null, null))
        .toList();
  }
  ...
  void upsertMember(UUID albumId, UUID userId, String role) {
    long updated =
        queryFactory
            .update(MEMBERS)
            .set(MEMBERS.memberRole, role)
            .where(MEMBERS.albumId.eq(albumId).and(MEMBERS.userId.eq(userId)))
            .execute();
    if (updated == 0)
      queryFactory
          .insert(MEMBERS)
          .set(MEMBERS.albumId, albumId)
          .set(MEMBERS.userId, userId)
          .set(MEMBERS.memberRole, role)
          .set(MEMBERS.createdAt, OffsetDateTime.now())
          .execute();
  }

  void removeMember(UUID albumId, UUID userId) {
    queryFactory
        .delete(MEMBERS)
        .where(MEMBERS.albumId.eq(albumId).and(MEMBERS.userId.eq(userId)))
        .execute();
  ...
  boolean ownsAll(UUID userId, List<UUID> albumIds) {
    Long count =
        queryFactory
            .select(ALBUMS.id.count())
            .from(ALBUMS)
            .where(ALBUMS.ownerUserId.eq(userId).and(ALBUMS.id.in(albumIds)))
            .fetchOne();
  ...
  Optional<UUID> createStarredIfAbsent(UUID userId) {
    ...
    boolean inserted =
        queryFactory
                ...
                .set(ALBUMS.id, id)
                .set(ALBUMS.ownerUserId, userId)
                .set(ALBUMS.name, AlbumKind.STARRED_NAME)
                .set(ALBUMS.kind, AlbumKind.STARRED)
                .set(ALBUMS.createdAt, now)
                .set(ALBUMS.updatedAt, now)
                .addFlag(
                    QueryFlag.Position.END,
                    " ON CONFLICT (owner_user_id) WHERE (kind = 'starred') DO NOTHING")
                .execute()
            > 0;
  ...
  boolean delete(UUID id, UUID userId) {
    return queryFactory
            .delete(ALBUMS)
            .where(ALBUMS.id.eq(id).and(ALBUMS.ownerUserId.eq(userId)))
            .execute()
        > 0;
  }
  ...
  int nextPosition(UUID albumId) {
    Integer max =
        queryFactory
            .select(ITEMS.position.max())
            .from(ITEMS)
            .where(ITEMS.albumId.eq(albumId))
            .fetchOne();
  ...
  boolean add(UUID albumId, UUID assetId, UUID libraryId, int position, UUID userId) {
    ...
    boolean inserted =
        queryFactory
                ...
                .set(ITEMS.sourceLibraryId, libraryId)
                .set(ITEMS.position, position)
                .set(ITEMS.addedBy, userId)
  ...
  void deleteItems(UUID albumId, List<UUID> assetIds) {
    if (queryFactory
            .delete(ITEMS)
            .where(ITEMS.albumId.eq(albumId).and(ITEMS.assetId.in(assetIds)))
            .execute()
        > 0) {
      queryFactory
          .update(ALBUMS)
          .set(ALBUMS.updatedAt, OffsetDateTime.now())
  ...
  private AlbumRecord record(com.querydsl.core.Tuple row) {
    ...
    return new AlbumRecord(
        row.get(ALBUMS.id),
        row.get(ALBUMS.name),
        row.get(ALBUMS.coverAssetId),
        row.get(FILES.fileName.min()),
        row.get(ALBUMS.kind),
        Math.toIntExact(itemCount == null ? 0 : itemCount),
        Math.toIntExact(libraryCount == null ? 0 : libraryCount),
        row.get(ALBUMS.createdAt),
        row.get(ALBUMS.updatedAt));
  }
