...
public class AlbumController {
  ...
  List<AlbumSummaryResponse> list(
  ...
  AlbumSummaryResponse create(
      @RequestBody CreateAlbumRequest request, @AuthenticationPrincipal AuthenticatedUser user) {
    return albumService.create(request, user);
  }

  @GetMapping("/api/albums/{albumId}")
  AlbumSummaryResponse get(
      @PathVariable UUID albumId, @AuthenticationPrincipal AuthenticatedUser user) {
    return albumService.get(albumId, user);
  }

  @PatchMapping("/api/albums/{albumId}")
  AlbumSummaryResponse update(
  ...
  AssetBrowseResponse starredAssets(
      @AuthenticationPrincipal AuthenticatedUser user,
      @RequestParam(required = false) Integer page,
      @RequestParam(required = false) Integer pageSize) {
  ...
  @ResponseStatus(HttpStatus.NO_CONTENT)
  void addItems(
      @RequestBody AddAlbumItemsRequest request, @AuthenticationPrincipal AuthenticatedUser user) {
  ...
  void deleteItems(
      @PathVariable UUID albumId,
      @RequestBody DeleteAlbumItemsRequest request,
      @AuthenticationPrincipal AuthenticatedUser user) {
    albumService.deleteItems(albumId, request, user);
  }

  @GetMapping("/api/albums/{albumId}/members")
  List<AlbumRepository.AlbumMemberRecord> members(
      @PathVariable UUID albumId, @AuthenticationPrincipal AuthenticatedUser user) {
    return albumService.members(albumId, user);
  }

  @GetMapping("/api/albums/{albumId}/members/candidates")
  List<AlbumRepository.AlbumMemberRecord> memberCandidates(
      @PathVariable UUID albumId, @AuthenticationPrincipal AuthenticatedUser user) {
    return albumService.memberCandidates(albumId, user);
  }

  @PostMapping("/api/albums/{albumId}/members")
  AlbumRepository.AlbumMemberRecord addMember(
      @PathVariable UUID albumId,
      @RequestBody UpsertAlbumMemberRequest request,
      @AuthenticationPrincipal AuthenticatedUser user) {
    return albumService.addMember(albumId, request, user);
  }

  @DeleteMapping("/api/albums/{albumId}/members/{userId}")
  @ResponseStatus(HttpStatus.NO_CONTENT)
  void removeMember(
      @PathVariable UUID albumId,
      @PathVariable UUID userId,
      @AuthenticationPrincipal AuthenticatedUser user) {
    albumService.removeMember(albumId, userId, user);
  }
}
