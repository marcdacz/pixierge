...
final class ProvisionalIdentity {
  ...
  static String fingerprint(String normalizedPath, long sizeBytes, OffsetDateTime modifiedAt) {
    String material =
        normalizedPath + "|" + sizeBytes + "|" + modifiedAt.toInstant().toEpochMilli();
    return PREFIX + digest(material);
  }

  static boolean pathUnchanged(
      ScanRepository.AssetFileRecord existing, long sizeBytes, OffsetDateTime modifiedAt) {
