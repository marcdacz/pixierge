...
class ScanRepository {
  ...
  boolean isScanCatalogCompleted(UUID scanRunId) {
    OffsetDateTime completedAt =
        queryFactory
            .select(SCAN_RUNS.catalogCompletedAt)
            .from(SCAN_RUNS)
            .where(SCAN_RUNS.id.eq(scanRunId).and(SCAN_RUNS.status.eq("running")))
            .fetchOne();
  ...
  Optional<ScanRunRecord> findScanRun(UUID scanRunId) {
    Tuple row = selectScanRuns().where(SCAN_RUNS.id.eq(scanRunId)).fetchOne();
  ...
  List<ActiveScanRecord> listActiveScanRuns() {
    return queryFactory
        ...
        .from(SCAN_RUNS)
        .join(LIBRARIES)
        .on(LIBRARIES.id.eq(SCAN_RUNS.libraryId))
        .leftJoin(LIBRARY_ROOTS)
        .on(LIBRARY_ROOTS.id.eq(SCAN_RUNS.rootId))
  ...
  List<ScanErrorRecord> errorsForScan(UUID scanRunId) {
    return queryFactory
        .select(
            SCAN_ERRORS.id,
            SCAN_ERRORS.path,
            SCAN_ERRORS.errorCode,
            SCAN_ERRORS.message,
            SCAN_ERRORS.createdAt)
        .from(SCAN_ERRORS)
  ...
  Optional<AssetFileRecord> findAssetFile(UUID assetFileId) {
    Tuple row = selectAssetFiles().where(ASSET_FILES.id.eq(assetFileId)).fetchOne();
    return Optional.ofNullable(row).map(this::toAssetFileRecord);
  }
            ...
            ASSET_FILES
                ...
                .and(
                    ASSET_FILES
                        .lastSeenScanRunId
                        .isNull()
                        .or(ASSET_FILES.lastSeenScanRunId.ne(scanRunId))))
        .fetch()
        .stream()
        .map(this::toAssetFileRecord)
        .toList();
  ...
  void createError(
      UUID scanRunId, UUID libraryId, UUID rootId, String path, String errorCode, String message) {
