...
public record ScanIdentityJobPayload(
  ...
  public List<ScanIdentityJobItem> identityItems() {
    ...
    return List.of(
        new ScanIdentityJobItem(
            rootId, assetFileId, path, normalizedPath, fileName, size, modifiedAt));
  }
  ...
  public UUID completionRootId() {
    ...
    return identityItems.stream().allMatch(item -> item.rootId().equals(firstRootId))
        ? firstRootId
        : null;
  }
}
