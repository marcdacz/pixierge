...
public class QFileObservations extends RelationalPathBase<QFileObservations> {
  ...
  public static final QFileObservations fileObservations =
      new QFileObservations("file_observations");
  ...
  public final DateTimePath<OffsetDateTime> observedAt =
      createDateTime("observedAt", OffsetDateTime.class);
  public final NumberPath<Long> processingDurationMs =
      createNumber("processingDurationMs", Long.class);
  ...
  private void addMetadata() {
    ...
    addMetadata(
        scanRunId, ColumnMetadata.named("scan_run_id").withIndex(2).ofType(Types.OTHER).notNull());
    addMetadata(
    ...
    addMetadata(
        assetFileId, ColumnMetadata.named("asset_file_id").withIndex(6).ofType(Types.OTHER));
    addMetadata(path, ColumnMetadata.named("path").withIndex(7).ofType(Types.VARCHAR).notNull());
    addMetadata(
        normalizedPath,
        ColumnMetadata.named("normalized_path").withIndex(8).ofType(Types.VARCHAR).notNull());
    addMetadata(sizeBytes, ColumnMetadata.named("size_bytes").withIndex(9).ofType(Types.BIGINT));
    addMetadata(
        modifiedAt,
        ColumnMetadata.named("modified_at").withIndex(10).ofType(Types.TIMESTAMP_WITH_TIMEZONE));
    addMetadata(
        partialHash, ColumnMetadata.named("partial_hash").withIndex(11).ofType(Types.VARCHAR));
    addMetadata(
        contentHash, ColumnMetadata.named("content_hash").withIndex(12).ofType(Types.VARCHAR));
    addMetadata(
        result, ColumnMetadata.named("result").withIndex(13).ofType(Types.VARCHAR).notNull());
    addMetadata(
        observedAt,
        ColumnMetadata.named("observed_at")
            .withIndex(14)
            .ofType(Types.TIMESTAMP_WITH_TIMEZONE)
            .notNull());
    addMetadata(
        processingDurationMs,
        ColumnMetadata.named("processing_duration_ms").withIndex(15).ofType(Types.BIGINT));
  }
