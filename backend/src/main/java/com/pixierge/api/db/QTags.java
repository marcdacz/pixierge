...
public class QTags extends RelationalPathBase<QTags> {
  ...
  private void addMetadata() {
    ...
    addMetadata(
        normalizedName,
        ColumnMetadata.named("normalized_name").withIndex(4).ofType(Types.VARCHAR).notNull());
    addMetadata(
        createdBy, ColumnMetadata.named("created_by").withIndex(5).ofType(Types.OTHER).notNull());
    addMetadata(
        createdAt,
