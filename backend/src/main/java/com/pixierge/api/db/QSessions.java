...
public class QSessions extends RelationalPathBase<QSessions> {
  ...
  public final DateTimePath<OffsetDateTime> expiresAt =
      createDateTime("expiresAt", OffsetDateTime.class);
  public final DateTimePath<OffsetDateTime> revokedAt =
      createDateTime("revokedAt", OffsetDateTime.class);
  public final DateTimePath<OffsetDateTime> createdAt =
      createDateTime("createdAt", OffsetDateTime.class);
  public final DateTimePath<OffsetDateTime> lastSeenAt =
      createDateTime("lastSeenAt", OffsetDateTime.class);
  ...
  private void addMetadata() {
    ...
    addMetadata(
        tokenHash, ColumnMetadata.named("token_hash").withIndex(3).ofType(Types.VARCHAR).notNull());
    addMetadata(
        csrfToken, ColumnMetadata.named("csrf_token").withIndex(4).ofType(Types.VARCHAR).notNull());
    addMetadata(
        expiresAt,
        ColumnMetadata.named("expires_at")
            .withIndex(5)
            .ofType(Types.TIMESTAMP_WITH_TIMEZONE)
            .notNull());
    addMetadata(
        revokedAt,
        ColumnMetadata.named("revoked_at").withIndex(6).ofType(Types.TIMESTAMP_WITH_TIMEZONE));
    addMetadata(
        createdAt,
    ...
    addMetadata(
        lastSeenAt,
        ColumnMetadata.named("last_seen_at")
            .withIndex(8)
            .ofType(Types.TIMESTAMP_WITH_TIMEZONE)
            .notNull());
  }
