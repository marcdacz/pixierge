...
public class QScanErrors extends RelationalPathBase<QScanErrors> {
    ...
    private void addMetadata() {
        ...
        addMetadata(errorCode, ColumnMetadata.named("error_code").withIndex(6).ofType(Types.VARCHAR).notNull());
        addMetadata(message, ColumnMetadata.named("message").withIndex(7).ofType(Types.VARCHAR).notNull());
        addMetadata(createdAt, ColumnMetadata.named("created_at").withIndex(8).ofType(Types.TIMESTAMP_WITH_TIMEZONE).notNull());
    }
