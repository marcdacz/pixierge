...
public class QScheduledJobs extends RelationalPathBase<QScheduledJobs> {
  ...
  public final DateTimePath<OffsetDateTime> lastRunAt =
      createDateTime("lastRunAt", OffsetDateTime.class);
  ...
  private void addMetadata() {
    ...
    addMetadata(
        jobKey, ColumnMetadata.named("job_key").withIndex(2).ofType(Types.VARCHAR).notNull());
    addMetadata(
        displayName,
        ColumnMetadata.named("display_name").withIndex(3).ofType(Types.VARCHAR).notNull());
    addMetadata(
        description,
        ColumnMetadata.named("description").withIndex(4).ofType(Types.VARCHAR).notNull());
    addMetadata(
        ownerType, ColumnMetadata.named("owner_type").withIndex(5).ofType(Types.VARCHAR).notNull());
    addMetadata(
        enabled, ColumnMetadata.named("enabled").withIndex(6).ofType(Types.BOOLEAN).notNull());
    addMetadata(
        cronExpression,
        ColumnMetadata.named("cron_expression").withIndex(7).ofType(Types.VARCHAR).notNull());
    addMetadata(
        timezone, ColumnMetadata.named("timezone").withIndex(8).ofType(Types.VARCHAR).notNull());
    addMetadata(
        nextRunAt,
        ColumnMetadata.named("next_run_at").withIndex(9).ofType(Types.TIMESTAMP_WITH_TIMEZONE));
    addMetadata(
        lastRunAt,
        ColumnMetadata.named("last_run_at").withIndex(10).ofType(Types.TIMESTAMP_WITH_TIMEZONE));
    addMetadata(
        lastStatus, ColumnMetadata.named("last_status").withIndex(11).ofType(Types.VARCHAR));
    addMetadata(
        timeoutSeconds,
        ColumnMetadata.named("timeout_seconds").withIndex(12).ofType(Types.INTEGER).notNull());
    addMetadata(
        concurrencyKey,
        ColumnMetadata.named("concurrency_key").withIndex(13).ofType(Types.VARCHAR).notNull());
    addMetadata(
        createdAt,
