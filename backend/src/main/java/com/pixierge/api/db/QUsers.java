...
public class QUsers extends RelationalPathBase<QUsers> {
  ...
  private void addMetadata() {
    ...
    addMetadata(
        username, ColumnMetadata.named("username").withIndex(2).ofType(Types.VARCHAR).notNull());
    addMetadata(
        status, ColumnMetadata.named("status").withIndex(3).ofType(Types.VARCHAR).notNull());
    addMetadata(
        createdAt,
