...
public class QAppMetadata extends RelationalPathBase<QAppMetadata> {
  ...
  private void addMetadata() {
    addMetadata(
        metadataKey,
        ColumnMetadata.named("metadata_key").withIndex(1).ofType(Types.VARCHAR).notNull());
    addMetadata(
        metadataValue,
        ColumnMetadata.named("metadata_value").withIndex(2).ofType(Types.VARCHAR).notNull());
    addMetadata(
        createdAt,
