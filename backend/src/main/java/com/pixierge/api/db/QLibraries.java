...
public class QLibraries extends RelationalPathBase<QLibraries> {
  ...
  public final DateTimePath<OffsetDateTime> archivedAt =
      createDateTime("archivedAt", OffsetDateTime.class);
  ...
  private void addMetadata() {
    ...
    addMetadata(
        ...
        ColumnMetadata.named("updated_at")
            ...
            .notNull());
    addMetadata(
        status, ColumnMetadata.named("status").withIndex(6).ofType(Types.VARCHAR).notNull());
    addMetadata(
        archivedAt,
        ColumnMetadata.named("archived_at").withIndex(7).ofType(Types.TIMESTAMP_WITH_TIMEZONE));
  }
