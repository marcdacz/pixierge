...
public class QAlbumItems extends RelationalPathBase<QAlbumItems> {
  ...
  private void addMetadata() {
    ...
    addMetadata(
        sourceLibraryId,
        ColumnMetadata.named("source_library_id").withIndex(3).ofType(Types.OTHER).notNull());
    addMetadata(
        position, ColumnMetadata.named("position").withIndex(4).ofType(Types.INTEGER).notNull());
    addMetadata(
        addedBy, ColumnMetadata.named("added_by").withIndex(5).ofType(Types.OTHER).notNull());
    addMetadata(
        createdAt,
