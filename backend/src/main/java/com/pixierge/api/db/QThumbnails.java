...
public class QThumbnails extends RelationalPathBase<QThumbnails> {
  ...
  public final DateTimePath<OffsetDateTime> generatedAt =
      createDateTime("generatedAt", OffsetDateTime.class);
  ...
  private void addMetadata() {
    ...
    addMetadata(
        contentHash,
        ColumnMetadata.named("content_hash").withIndex(3).ofType(Types.VARCHAR).notNull());
    addMetadata(
        thumbnailType,
        ColumnMetadata.named("thumbnail_type").withIndex(4).ofType(Types.VARCHAR).notNull());
    addMetadata(width, ColumnMetadata.named("width").withIndex(5).ofType(Types.INTEGER).notNull());
    addMetadata(
        height, ColumnMetadata.named("height").withIndex(6).ofType(Types.INTEGER).notNull());
    addMetadata(
        format, ColumnMetadata.named("format").withIndex(7).ofType(Types.VARCHAR).notNull());
    addMetadata(
        generatorVersion,
        ColumnMetadata.named("generator_version").withIndex(8).ofType(Types.VARCHAR).notNull());
    addMetadata(
        configVersion,
        ColumnMetadata.named("config_version").withIndex(9).ofType(Types.VARCHAR).notNull());
    addMetadata(
        cacheKey, ColumnMetadata.named("cache_key").withIndex(10).ofType(Types.VARCHAR).notNull());
    addMetadata(
        relativePath,
        ColumnMetadata.named("relative_path").withIndex(11).ofType(Types.VARCHAR).notNull());
    addMetadata(
        byteSize, ColumnMetadata.named("byte_size").withIndex(12).ofType(Types.BIGINT).notNull());
    addMetadata(
        placeholder, ColumnMetadata.named("placeholder").withIndex(13).ofType(Types.VARCHAR));
    addMetadata(
        status, ColumnMetadata.named("status").withIndex(14).ofType(Types.VARCHAR).notNull());
    addMetadata(
        errorMessage, ColumnMetadata.named("error_message").withIndex(15).ofType(Types.VARCHAR));
    addMetadata(
        createdAt,
    ...
    addMetadata(
        generatedAt,
        ColumnMetadata.named("generated_at").withIndex(18).ofType(Types.TIMESTAMP_WITH_TIMEZONE));
  }
