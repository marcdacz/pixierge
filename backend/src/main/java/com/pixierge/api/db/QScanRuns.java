...
public class QScanRuns extends RelationalPathBase<QScanRuns> {
  ...
  public final DateTimePath<OffsetDateTime> catalogCompletedAt =
      createDateTime("catalogCompletedAt", OffsetDateTime.class);
  public final NumberPath<Integer> scannedFileCount =
      createNumber("scannedFileCount", Integer.class);
  ...
  private void addMetadata() {
    ...
    addMetadata(
        status, ColumnMetadata.named("status").withIndex(5).ofType(Types.VARCHAR).notNull());
    addMetadata(
        startedAt,
        ColumnMetadata.named("started_at")
            .withIndex(6)
            .ofType(Types.TIMESTAMP_WITH_TIMEZONE)
            .notNull());
    addMetadata(
        completedAt,
        ColumnMetadata.named("completed_at").withIndex(7).ofType(Types.TIMESTAMP_WITH_TIMEZONE));
    addMetadata(
        scannedFileCount,
        ColumnMetadata.named("scanned_file_count").withIndex(8).ofType(Types.INTEGER).notNull());
    addMetadata(
        addedCount,
        ColumnMetadata.named("added_count").withIndex(9).ofType(Types.INTEGER).notNull());
    addMetadata(
        unchangedCount,
        ColumnMetadata.named("unchanged_count").withIndex(10).ofType(Types.INTEGER).notNull());
    addMetadata(
        movedCount,
        ColumnMetadata.named("moved_count").withIndex(11).ofType(Types.INTEGER).notNull());
    addMetadata(
        modifiedCount,
        ColumnMetadata.named("modified_count").withIndex(12).ofType(Types.INTEGER).notNull());
    addMetadata(
        duplicateCount,
        ColumnMetadata.named("duplicate_count").withIndex(13).ofType(Types.INTEGER).notNull());
    addMetadata(
        missingCount,
        ColumnMetadata.named("missing_count").withIndex(14).ofType(Types.INTEGER).notNull());
    addMetadata(
        reappearedCount,
        ColumnMetadata.named("reappeared_count").withIndex(15).ofType(Types.INTEGER).notNull());
    addMetadata(
        errorCount,
        ColumnMetadata.named("error_count").withIndex(16).ofType(Types.INTEGER).notNull());
    addMetadata(
        catalogCompletedAt,
        ColumnMetadata.named("catalog_completed_at")
            .withIndex(17)
            .ofType(Types.TIMESTAMP_WITH_TIMEZONE));
  }
