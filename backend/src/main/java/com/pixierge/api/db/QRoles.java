...
public class QRoles extends RelationalPathBase<QRoles> {
  ...
  private void addMetadata() {
    ...
    addMetadata(
        roleKey, ColumnMetadata.named("role_key").withIndex(2).ofType(Types.VARCHAR).notNull());
    ...
    addMetadata(
        builtIn, ColumnMetadata.named("built_in").withIndex(5).ofType(Types.BOOLEAN).notNull());
    addMetadata(
        createdAt,
