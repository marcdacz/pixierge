...
public class QScheduledJobRuns extends RelationalPathBase<QScheduledJobRuns> {
  ...
  public static final QScheduledJobRuns scheduledJobRuns =
      new QScheduledJobRuns("scheduled_job_runs");
  ...
  public final DateTimePath<OffsetDateTime> finishedAt =
      createDateTime("finishedAt", OffsetDateTime.class);
  ...
  private void addMetadata() {
    ...
    addMetadata(
        triggerSource,
        ColumnMetadata.named("trigger_source").withIndex(3).ofType(Types.VARCHAR).notNull());
    addMetadata(
    ...
    addMetadata(
        finishedAt,
        ColumnMetadata.named("finished_at").withIndex(6).ofType(Types.TIMESTAMP_WITH_TIMEZONE));
    addMetadata(durationMs, ColumnMetadata.named("duration_ms").withIndex(7).ofType(Types.BIGINT));
    addMetadata(
        summaryJson, ColumnMetadata.named("summary_json").withIndex(8).ofType(Types.VARCHAR));
    addMetadata(
        errorMessage, ColumnMetadata.named("error_message").withIndex(9).ofType(Types.VARCHAR));
    addMetadata(
        createdAt,
