...
public class QBackgroundJobs extends RelationalPathBase<QBackgroundJobs> {
  ...
  public final DateTimePath<OffsetDateTime> nextRunAt =
      createDateTime("nextRunAt", OffsetDateTime.class);
  public final DateTimePath<OffsetDateTime> leaseUntil =
      createDateTime("leaseUntil", OffsetDateTime.class);
  ...
  public final DateTimePath<OffsetDateTime> completedAt =
      createDateTime("completedAt", OffsetDateTime.class);
  public final DateTimePath<OffsetDateTime> startedAt =
      createDateTime("startedAt", OffsetDateTime.class);
  ...
  private void addMetadata() {
    ...
    addMetadata(
        jobType, ColumnMetadata.named("job_type").withIndex(2).ofType(Types.VARCHAR).notNull());
    addMetadata(
        payloadJson,
        ColumnMetadata.named("payload_json").withIndex(3).ofType(Types.VARCHAR).notNull());
    addMetadata(
        status, ColumnMetadata.named("status").withIndex(4).ofType(Types.VARCHAR).notNull());
    addMetadata(
        priority, ColumnMetadata.named("priority").withIndex(5).ofType(Types.INTEGER).notNull());
    addMetadata(
        attempts, ColumnMetadata.named("attempts").withIndex(6).ofType(Types.INTEGER).notNull());
    addMetadata(
        maxAttempts,
        ColumnMetadata.named("max_attempts").withIndex(7).ofType(Types.INTEGER).notNull());
    addMetadata(
        nextRunAt,
        ColumnMetadata.named("next_run_at")
            .withIndex(8)
            .ofType(Types.TIMESTAMP_WITH_TIMEZONE)
            .notNull());
    addMetadata(
        leaseUntil,
        ColumnMetadata.named("lease_until").withIndex(9).ofType(Types.TIMESTAMP_WITH_TIMEZONE));
    addMetadata(lockedBy, ColumnMetadata.named("locked_by").withIndex(10).ofType(Types.VARCHAR));
    addMetadata(
        concurrencyKey,
        ColumnMetadata.named("concurrency_key").withIndex(11).ofType(Types.VARCHAR).notNull());
    addMetadata(dedupeKey, ColumnMetadata.named("dedupe_key").withIndex(12).ofType(Types.VARCHAR));
    addMetadata(
        progressJson, ColumnMetadata.named("progress_json").withIndex(13).ofType(Types.VARCHAR));
    addMetadata(
        lastErrorCode, ColumnMetadata.named("last_error_code").withIndex(14).ofType(Types.VARCHAR));
    addMetadata(
        lastErrorMessage,
        ColumnMetadata.named("last_error_message").withIndex(15).ofType(Types.VARCHAR));
    addMetadata(
        createdAt,
    ...
    addMetadata(
        completedAt,
        ColumnMetadata.named("completed_at").withIndex(18).ofType(Types.TIMESTAMP_WITH_TIMEZONE));
    addMetadata(
        startedAt,
        ColumnMetadata.named("started_at").withIndex(19).ofType(Types.TIMESTAMP_WITH_TIMEZONE));
  }
