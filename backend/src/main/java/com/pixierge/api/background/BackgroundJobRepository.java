...
public class BackgroundJobRepository {
  ...
  public UUID enqueue(BackgroundJobCreate create, OffsetDateTime now) {
    ...
    return findActiveByDedupeKey(create.dedupeKey()).map(BackgroundJobRecord::id).orElse(id);
  }

  @Transactional
  public List<BackgroundJobRecord> claimReadyJobs(
      int limit, String workerId, OffsetDateTime now, OffsetDateTime leaseUntil) {
  ...
  private List<Tuple> keepClaimableConcurrencyKeys(
      List<Tuple> rows, int limit, OffsetDateTime now) {
  ...
  private boolean hasActiveRunningJobForKey(
      String concurrencyKey, UUID ignoredJobId, OffsetDateTime now) {
    return queryFactory
  ...
  public void heartbeat(
      UUID jobId, String workerId, OffsetDateTime leaseUntil, OffsetDateTime now) {
    queryFactory
        .update(JOBS)
        .set(JOBS.leaseUntil, leaseUntil)
        .set(JOBS.updatedAt, now)
  ...
  public void cancel(UUID jobId, OffsetDateTime now) {
    queryFactory
        ...
        .set(JOBS.completedAt, now)
        .set(JOBS.updatedAt, now)
        .where(JOBS.id.eq(jobId).and(JOBS.status.in(STATUS_PENDING, STATUS_RUNNING)))
        .execute();
  }

  @Transactional(readOnly = true)
  public Optional<BackgroundJobRecord> find(UUID jobId) {
    Tuple row = selectJobs().where(JOBS.id.eq(jobId)).fetchOne();
  ...
  public Optional<BackgroundJobRecord> findActiveByDedupeKey(String dedupeKey) {
    ...
    Tuple row =
        selectJobs()
            .where(JOBS.dedupeKey.eq(dedupeKey).and(JOBS.status.in(STATUS_PENDING, STATUS_RUNNING)))
            .fetchFirst();
    return Optional.ofNullable(row).map(this::toRecord);
  }
  ...
  public boolean hasActiveJobs(String jobType, String dedupeKeyPrefix, UUID excludedJobId) {
    var predicate = JOBS.jobType.eq(jobType).and(JOBS.status.in(STATUS_PENDING, STATUS_RUNNING));
    ...
    Integer exists = queryFactory.selectOne().from(JOBS).where(predicate).fetchFirst();
    return exists != null;
  }
  ...
  public List<BackgroundJobStatusSummary> summarizeByTypeAndStatus() {
    ...
    return queryFactory
        .select(
            JOBS.jobType, JOBS.status, jobCount, oldestCreatedAt, oldestNextRunAt, latestUpdatedAt)
        .from(JOBS)
        ...
        .map(
            row ->
                new BackgroundJobStatusSummary(
  ...
  public List<BackgroundJobProblemSummary> latestProblemJobs(int limit) {
    return selectJobs()
        .where(JOBS.status.in(STATUS_FAILED, STATUS_DEAD_LETTER).or(JOBS.lastErrorCode.isNotNull()))
  ...
  public List<BackgroundJobRecord> latestJobs(int limit) {
    return selectJobs().orderBy(JOBS.updatedAt.desc()).limit(Math.max(1, limit)).fetch().stream()
        .map(this::toRecord)
        .toList();
  }
