...
final class BackgroundFileActivityPager {
  ...
  static BackgroundFileActivityPage page(
      ...
      int page,
      int pageSize,
      BiFunction<Integer, Integer, BackgroundActivityRepository.PersistedFileActivityPage>
          persistedPage) {
    ...
    return new BackgroundFileActivityPage(
        List.copyOf(items), normalizedPage, normalizedPageSize, totalCount, hasNext);
  }
  ...
  static boolean matches(
    ...
    if (q != null) {
      String haystack =
          ((item.path() == null ? "" : item.path()) + " " + item.fileName())
    ...
    if (updatedFrom != null
        && (item.updatedAt() == null || item.updatedAt().isBefore(updatedFrom))) {
      return false;
    }
