...
public class BackgroundJobService {
  ...
  public BackgroundJobService(
      BackgroundJobRepository repository, TransactionTemplate transactionTemplate) {
    this(repository, transactionTemplate, Clock.systemUTC(), null);
  }

  BackgroundJobService(
      BackgroundJobRepository repository, TransactionTemplate transactionTemplate, Clock clock) {
  ...
  public List<BackgroundJobRecord> claimReadyJobs(
      int limit, String workerId, String includedJobType, String excludedJobType) {
    OffsetDateTime now = now();
    ...
    return repository.claimReadyJobs(
        limit, workerId, now, now.plus(DEFAULT_LEASE_DURATION), includedJobType, excludedJobType);
  }

  public void heartbeat(UUID jobId, String workerId) {
    OffsetDateTime now = now();
    transactionTemplate.executeWithoutResult(
        status -> repository.heartbeat(jobId, workerId, now.plus(DEFAULT_LEASE_DURATION), now));
  }

  public void complete(UUID jobId, String workerId, String progressJson) {
    OffsetDateTime now = now();
    transactionTemplate.executeWithoutResult(
        status -> repository.complete(jobId, workerId, progressJson, now));
  }
  ...
  public void fail(UUID jobId, String workerId, String errorCode, String errorMessage) {
    ...
    transactionTemplate.executeWithoutResult(
        status -> {
          ...
          repository.retry(
              jobId, workerId, errorCode, errorMessage, now.plus(retryDelay(job.attempts())), now);
        });
  }
  ...
  public boolean hasActiveJobs(String jobType, String dedupeKeyPrefix, UUID excludedJobId) {
    return transactionTemplate.execute(
        status -> repository.hasActiveJobs(jobType, dedupeKeyPrefix, excludedJobId));
  }
