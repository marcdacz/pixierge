...
class BackgroundActivityRepository {
  ...
  PersistedFileActivityPage searchFileActivity(
    ...
    List<BackgroundFileActivityRow> items =
        queryFactory
            .select(
                EVENTS.assetId,
                EVENTS.path,
            ...
            .from(EVENTS)
            .where(where)
            .orderBy(EVENTS.occurredAt.desc(), EVENTS.path.asc())
            ...
            .map(
                row ->
                    new BackgroundFileActivityRow(
                        row.get(EVENTS.assetId),
                        row.get(EVENTS.path),
                        row.get(EVENTS.status),
                        row.get(EVENTS.occurredAt),
                        row.get(EVENTS.message),
                        row.get(EVENTS.durationMs)))
            .toList();
  ...
  @Transactional
  int clear() {
    return (int) queryFactory.delete(EVENTS).execute();
  }

  @Transactional
  long deleteOlderThan(OffsetDateTime cutoff) {
    return queryFactory.delete(EVENTS).where(EVENTS.occurredAt.lt(cutoff)).execute();
  }
  ...
  void create(
    ...
    queryFactory
        ...
        .set(EVENTS.assetId, assetId)
        .set(EVENTS.path, path)
        .set(EVENTS.status, status)
        .set(EVENTS.occurredAt, occurredAt)
        .set(EVENTS.message, message)
        .set(EVENTS.durationMs, durationMs)
        .set(EVENTS.jobId, jobId)
        .set(EVENTS.batchLabel, batchLabel)
        .execute();
  }

  private BooleanBuilder where(
      String q, Collection<String> statuses, OffsetDateTime from, OffsetDateTime to) {
    BooleanBuilder where = new BooleanBuilder();
    if (q != null && !q.isBlank())
      where.and(EVENTS.path.lower().contains(q.trim().toLowerCase(Locale.ROOT)));
