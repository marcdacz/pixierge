...
class BackgroundWorkController {
  ...
  BackgroundWorkController(
      ...
      @Value("${pixierge.background-jobs.max-concurrent-metadata-jobs:2}")
  ...
  BackgroundWorkActivityResponse activity(@RequestParam(defaultValue = "100") int limit) {
    ...
    List<BackgroundActivityJobSummary> jobSummaries =
        jobs.stream().map(this::toJobSummary).toList();
    ...
    if (files.size() < boundedLimit) {
      files.addAll(
          activityRepository.recentFileActivity(boundedLimit - files.size()).stream()
              .map(this::toFileSummary)
              .toList());
    }
    return new BackgroundWorkActivityResponse(
        jobSummaries, files.stream().limit(boundedLimit).toList());
  }
  ...
  BackgroundFileActivityPage files(
    ...
    List<BackgroundFileActivitySummary> activeMatches =
        backgroundJobService.latestJobs(50).stream()
            .flatMap(job -> activeFileRows(job, Integer.MAX_VALUE).stream())
            .filter(
                item ->
                    BackgroundFileActivityPager.matches(
                        item, normalizedQuery, normalizedStatuses, filterFrom, filterTo))
            .toList();
    ...
    return BackgroundFileActivityPager.page(
        ...
        pageSize,
        (offset, limit) ->
            activityRepository.searchFileActivity(
                normalizedQuery, normalizedStatuses, filterFrom, filterTo, offset, limit));
  }

  @DeleteMapping("/api/admin/background/files")
  BackgroundActivityClearResponse clearFiles() {
    return new BackgroundActivityClearResponse(activityRepository.clear());
  }
  ...
  private List<BackgroundFileActivitySummary> activeFileRows(BackgroundJobRecord job, int limit) {
    ...
    if (ScanJobTypes.ASSET_IDENTITY_BACKFILL.equals(job.jobType())) {
      return identityPayload(job).identityItems().stream()
          ...
          .map(
              item ->
                  new BackgroundFileActivitySummary(
                      null,
                      item.path(),
                      fileName(item.path()),
                      status,
  ...
  private Long metadataDuration(BackgroundJobRecord job) {
    ...
    return Math.max(
        0L, Duration.between(job.startedAt(), OffsetDateTime.now(ZoneOffset.UTC)).toMillis());
  }
