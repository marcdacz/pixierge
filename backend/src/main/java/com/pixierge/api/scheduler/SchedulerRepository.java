...
public class SchedulerRepository {
  ...
  private static final String ON_LOCK_CONFLICT_DO_NOTHING =
  ...
  public List<SchedulerJobRecord> findDueJobs(OffsetDateTime now) {
    return queryFactory
        .select(
            ...
            JOBS.updatedAt)
        .from(JOBS)
        .where(JOBS.enabled.isTrue().and(JOBS.nextRunAt.isNotNull()).and(JOBS.nextRunAt.loe(now)))
  ...
  public boolean tryAcquireLock(
    ...
    queryFactory
        .delete(LOCKS)
        .where(LOCKS.concurrencyKey.eq(concurrencyKey).and(LOCKS.acquiredAt.loe(staleBefore)))
        .execute();
    long inserted =
        queryFactory
            .insert(LOCKS)
  ...
  public void releaseLock(String concurrencyKey, UUID runId) {
    queryFactory
        .delete(LOCKS)
        .where(LOCKS.concurrencyKey.eq(concurrencyKey).and(LOCKS.runId.eq(runId)))
        .execute();
  }
