...
public class SchedulerService {
  ...
  public SchedulerService(
      ...
      @Qualifier("taskScheduler") TaskExecutor taskExecutor,
  ...
  public SchedulerJobResponse updateJob(UUID jobId, UpdateSchedulerJobRequest request) {
    ...
    OffsetDateTime nextRun =
        enabled ? CronSupport.nextRunAt(cronExpression, timezone, OffsetDateTime.now()) : null;

    transactionTemplate.executeWithoutResult(
        status -> repository.updateJobSchedule(jobId, enabled, cronExpression, timezone, nextRun));
    return getJob(jobId);
  }
  ...
  private SchedulerJobRunResponse startRun(SchedulerJobRecord job, String triggerSource) {
    ...
    UUID runId =
        transactionTemplate.execute(
            status -> {
              UUID createdRunId =
                  repository.insertRun(job.id(), triggerSource, STATUS_RUNNING, startedAt);
              OffsetDateTime staleBefore = startedAt.minusSeconds(job.timeoutSeconds());
              if (!repository.tryAcquireLock(
                  job.concurrencyKey(), job.id(), createdRunId, startedAt, staleBefore)) {
                repository.completeRun(
                    ...
                    0L,
                    null,
                    "Another run is already in progress for concurrency key "
                        + job.concurrencyKey());
                return null;
              }
              ...
              if (TRIGGER_SCHEDULED.equals(triggerSource)) {
                OffsetDateTime nextRun =
                    CronSupport.nextRunAt(job.cronExpression(), job.timezone(), startedAt);
                repository.advanceNextRunIfScheduleUnchanged(job, nextRun);
              }
              return createdRunId;
            });
    ...
    return repository
        ...
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.INTERNAL_SERVER_ERROR, "Run not found"));
  }
  ...
  private void executeRun(UUID jobId, UUID runId) {
    ...
    try {
      ...
      long durationMs =
          Math.max(0, finishedAt.toInstant().toEpochMilli() - startedAt.toInstant().toEpochMilli());
      ...
      transactionTemplate.executeWithoutResult(
          tx -> {
            repository.completeRun(
                runId, finalStatus, finishedAt, durationMs, finalSummary, finalError);
  ...
  private void failUnsubmittedRun(
      SchedulerJobRecord job, UUID runId, OffsetDateTime startedAt, RuntimeException exception) {
    OffsetDateTime finishedAt = OffsetDateTime.now();
    long durationMs =
        Math.max(0, finishedAt.toInstant().toEpochMilli() - startedAt.toInstant().toEpochMilli());
    String errorMessage =
  ...
  private SchedulerJobRecord requireJob(UUID jobId) {
    return repository
        .findJob(jobId)
        .orElseThrow(
            () -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Scheduler job not found"));
  }
}
