...
describe('StructuredSearch', () => {
  it('hydrates structured clauses from the initial value into pills', async () => {
    ...
    render(
      <StrictMode>
        <StructuredSearch onChange={onChange} onValidQueryChange={vi.fn()} value="extension:.png extension:.jpg" />
      </StrictMode>
    );
  ...
  it('collapses repeated library clauses into one comma-separated pill', async () => {
    ...
    render(<StructuredSearch onChange={onChange} onValidQueryChange={vi.fn()} value="library:Events library:Japan" />);
      ...
      vi.fn(async () =>
        jsonResponse({
          ...
          errors: [
            {
              code: 'UNKNOWN_FIELD',
              message: "Search field 'person' is unavailable until a plugin provides it",
              start: 0,
              end: 12
            }
          ],
